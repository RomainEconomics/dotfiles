...
function M.setup() vim.notify('prez.lua setup', 'info', {}) end
	...
	for _, line in ipairs(lines) do
		...
		if line == separator then
			...
			if current_slide.title == nil then
				if line ~= '' then current_slide.title = line end
			else
...
function M.start_prez(opts)
	...
	for_each_float(function(_, float) vim.bo[float.buf].filetype = 'markdown' end)
	...
	vim.api.nvim_create_autocmd('BufLeave', {
		...
		callback = function()
			...
			for_each_float(function(_, float) pcall(vim.api.nvim_win_close, float.win, true) end)
		end,
	})
	...
	vim.api.nvim_create_autocmd('VimResized', {
		...
		callback = function()
			...
			for_each_float(function(name, float) vim.api.nvim_win_set_config(float.win, updated_config[name]) end)
